...
CAM <- function(y_obser,                         # Observations, organized into
  ...
  N       <- nrow(y_obser)         # total number of observations
  ...
  # Containers ##########################################################################################################################
  theta                 <- matrix(NA, L0, 4)  # new: 4 cols
  cij                   <- numeric(N)
  zj                    <- numeric(J)
  ...
  cij       <- km.out$cluster
  theta[, 1]  <- tapply(y_obser[,1], cij, mean)
  theta[, 2]  <- tapply(y_obser[,1], cij, sd)
  theta[, 3]  <- tapply(y_obser[,2], cij, mean)
  theta[, 4]  <- tapply(y_obser[,2], cij, sd)
  if (K0 >= J) {
    zj        <- 1:J
  } else{
    km.outZ   <- kmeans(cbind(tapply(Y[,1], G, mean),
                              tapply(Y[,2], G, mean)), K0)  # use average value to cluster units by kmeans
    zj        <- km.outZ$cluster
  }
  zj.pg    <- rep(zj, nj)
  #####################################################################################
