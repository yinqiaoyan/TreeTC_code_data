...
#' @param sigma step size for slice construction       # width in "stepout" method for finding interval I
#' @param setpOut use setp out procedure or not        # stepout method: estimate the interval I = (L,R) (Neal2003)
...
SliceSampler <- function(initX,
  ...
  DirectionSlice <- function(direction, initX) {
    ...
    while(T) {
      stepsIn <- stepsIn + 1
      newZ <- (upper - lower) * runif(1) + lower  # newZ from Unif(lower, upper)
...
#' Sample from a user supplied distribution with Metropolis-Hastings method
#' @param initX initial parameter
#' @param postLogDist unnormalized log-posterior distribution
#' @param dLogProposal log-proposal density
#' @param rProposal random variable generator by proposal distribution
#' @param sd standard deviation in proposal
#' @param ... additional args for postLogDist
#' @return a sample of parameter
#' @export
MH <- function(initX, postLogDist, dLogProposal, rProposal, sd, ...) {
  propX <- rProposal(initX, sd)
  r <- postLogDist(propX, ...) +
    dLogProposal(initX, propX, sd) -
    postLogDist(initX, ...) -
    dLogProposal(propX, initX, sd)
  r <- exp(r)
  u <- runif(1)
  if (u < min(1,r)) {
    newX <- propX
