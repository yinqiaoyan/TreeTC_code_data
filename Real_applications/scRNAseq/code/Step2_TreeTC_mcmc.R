...
q0 <- Normal$new(priorSigmaScale = NULL,
                 priorSigmaMin = priorSigmaMin,
                 priorSigmaMax = priorSigmaMax,
...
for (i in 0:numOfMCMC) {
  ...
  tssb$root$node$ResampleHyperParams(Slice_sigma   = 1.0, 
                                     Slice_stepOut = FALSE)
...
groups_subj <- unique(finalSubjAssignments)
...
save(q0, tssb, testData, numOfTrees, numOfSubjData, etaNormal, groups_subj, 
     ggFinal1, ggFinal2, subFinalClIds1, subFinalClIds2,
     subFinalCenters1, subFinalCenters2, finalSigma, finalDrift,
     tssbNum, finalSubjAssignments, finalClIds, 
