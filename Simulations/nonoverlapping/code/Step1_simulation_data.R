...
for (s in 1:num_sources) {
  if (s %% 2 == 1) {
    tmpdat = rbind(rmvnorm(num_obs_each, mean = rep(0, dims), sigma = diag(2.0^2, dims, dims)),
                   rmvnorm(num_obs_each, mean = c(-12, 12), sigma = diag(1.0^2, dims, dims)),
                   rmvnorm(num_obs_each, mean = c(-12, 16), sigma = diag(0.5^2, dims, dims)),
                   rmvnorm(num_obs_each, mean = c(-16, 12), sigma = diag(0.5^2, dims, dims)),
                   rmvnorm(num_obs_each, mean = c(-12, -12), sigma = diag(1.0^2, dims, dims)),
                   rmvnorm(num_obs_each, mean = c(12, 12), sigma = diag(1.0^2, dims, dims)))
  } else {
    tmpdat = rbind(rmvnorm(num_obs_each, mean = rep(0, dims), sigma = diag(2.0^2, dims, dims)),
